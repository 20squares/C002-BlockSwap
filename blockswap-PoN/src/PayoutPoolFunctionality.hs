...
reportToPayoutPool reporterAddr report' s
 | not (preconditions reporterAddr report' s) = s
 | preconditions reporterAddr report' s && report'._penaltyType == ValidatorKicked = kickProposer report' s
 | preconditions reporterAddr report' s && report'._penaltyType == Validator = penalizeProposer reporterAddr report' s
 | preconditions reporterAddr report' s && report'._penaltyType == Builder = penalizeBuilder reporterAddr report' s

-- Kick proposer
-- NOTE this is incomplete; to be augmented when proposer and builder are addressed
kickProposer :: Report -> State -> State
kickProposer r = over (statePoNOnChain % proposerStatus ) (M.update (\_ -> Just ProposerKicked) (r._proposer))
...
penalizeProposer addr r s = slashProposer addr r $ reportProposer r s
 where
    ...
    slashProposer addr r = over (stateOnChain % payoutPool % reporterRegistry) (M.adjust (over rewards (+ (r._amount))) addr)
...
penalizeBuilder addr r s = slashBuilder addr r s
  where
    -- NOTE we focus on the payment to reporter part; adjustments to proposer tbd
    -- NOTE this is incomplete; to be augmented when proposer and builder are addressed
    slashBuilder addr r = over (stateOnChain % payoutPool % reporterRegistry) (M.adjust (over rewards (+ (r._amount))) addr)
...
registerReporter addr s =
  case M.lookup addr s._stateOnChain._payoutPool._reporterRegistry of
    ...
    Just r  -> (if (r._isActive) then s else over (stateOnChain % payoutPool % reporterRegistry) (M.insert addr reporter) s)
  where
    reporter = Reporter
       { _rewards           = 0
       , _isActive          = True
...
rageQuitReporter addr s =
  case M.lookup addr s._stateOnChain._payoutPool._reporterRegistry of
    ...
    Just r  -> case _lastReportedBlock r of
      Nothing                 -> s
      Just lastBlockReported  -> (if lastBlockReported + s._stateOnChain._payoutPool._reporterPayoutDelay < s._stateOnChain._block then over (stateOnChain % payoutPool % reporterRegistry) (M.adjust (set isRageQuitted True) addr) s else s)
...
claimReporterRewards addr s =
  ...
  where
    ...
    adjustBalancePayoutPool r' = over (stateOnChain % payoutPool % maintenanceBalance) (\balance -> balance - _rewards r')
...
preconditions reporterAddr report' s = and
  ...
  , (s._stateOnChain._payoutPool._reporterRegistry M.! reporterAddr)._isActive
  , not ((s._stateOnChain._payoutPool._reporterRegistry M.! reporterAddr)._isRageQuitted)
  , report'._slotId > s._stateOnChain._payoutPool._deploymentEpoch * 32
  , isNothing (M.lookup report'._slotId s._stateOnChain._payoutPool._reportsSlotsInUse)
  ]
